-- Security fixes for SECURITY DEFINER functions and privilege escalation (v2)
...
-- 4. Add missing updated_at triggers (check existence first)
DO $$
BEGIN
    -- Add trigger for notification_schedules if not exists
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_notification_schedules_updated_at') THEN
